(function (global, factory) {
  ...
  const padEnd = function (string, maxLength, fillString) {
    if (string == null || maxLength == null) {
      return string
    }

    const result = String(string)
    const targetLen = typeof maxLength === 'number'
      ? maxLength
  ...
  const MAX_SIGNIFICANT_DIGITS = 17 // Highest value you can safely put here is Number.MAX_SAFE_INTEGER-MAX_SIGNIFICANT_DIGITS

  const EXP_LIMIT = 9e15 // The largest exponent that can appear in a Number, though not all mantissas are valid here.

  const NUMBER_EXP_MAX = 308 // The smallest exponent that can appear in a Number, though not all mantissas are valid here.

  const NUMBER_EXP_MIN = -324 // Tolerance which is used for Number conversion to compensate floating-point error.
